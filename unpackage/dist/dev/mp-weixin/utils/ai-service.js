...
const config_aiConfig = require("../config/ai-config.js");
class AIService {
  constructor() {
    this.config = config_aiConfig.AI_CONFIG[config_aiConfig.CURRENT_CONFIG];
    this.commonConfig = config_aiConfig.AI_CONFIG.common;
  }
  // 主要的AI调用方法
  async chat(message, conversationHistory = []) {
    try {
      switch (config_aiConfig.CURRENT_CONFIG) {
      ...
      common_vendor.index.__f__("error", "at utils/ai-service.js:25", "AI调用失败:", error);
      throw error;
    }
  }
  // OpenAI兼容API调用
  ...
  async callWebSocket(message) {
    return new Promise((resolve, reject) => {
      const ws = common_vendor.index.connectSocket({
  ...
  async chatWithRetry(message, conversationHistory = []) {
    ...
    for (let i = 0; i < this.commonConfig.retryTimes; i++) {
      try {
        ...
        lastError = error;
        common_vendor.index.__f__("warn", "at utils/ai-service.js:175", `AI调用失败，第${i + 1}次重试:`, error);
...
const AIService$1 = new AIService();
exports.AIService = AIService$1;
//# sourceMappingURL=../../.sourcemap/mp-weixin/utils/ai-service.js.map
